...
public class PreConfiguredTokenizerTest extends PreTrainedProcessorTest<PreConfiguredTokenizer> {
    ...
    public void shouldTokenizeWithSimpleTokenizer() {
        ...
        flowFile.assertAttributeExists(NLPAttribute.TOKENIZER_TOKENS_LIST_KEY);
        flowFile.assertAttributeExists(NLPAttribute.TOKENIZER_TOKENS_SPAN_KEY);

        List<String> tokensList = NLPAttribute.get(NLPAttribute.TOKENIZER_TOKENS_LIST_KEY, flowFile.getAttributes(), new TypeToken<List<String>>() {
        });
        List<Span> tokenSpans = NLPAttribute.get(NLPAttribute.TOKENIZER_TOKENS_SPAN_KEY, flowFile.getAttributes(), new TypeToken<List<Span>>() {
        });
