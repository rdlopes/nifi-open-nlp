...
import static org.rdlopes.processors.opennlp.common.TokenizerType.WHITESPACE;
...
public class TrainedTokenizerTest extends AbstractPreTrainedProcessorTest<TrainedTokenizer> {

    public TrainedTokenizerTest() {
        super(TrainedTokenizer.class);
    }

    @Test
    public void shouldTokenizeWithSimpleTokenizer() {
        setModelFilePath("/models/en-token.bin");
        testRunner.setProperty(TOKENIZE_TOKENIZER_TYPE.descriptor, SIMPLE.name());
    ...
    public void shouldTokenizeWithWhitespaceTokenizer() {
        setModelFilePath("/models/en-token.bin");
        testRunner.setProperty(TOKENIZE_TOKENIZER_TYPE.descriptor, WHITESPACE.name());
        testRunner.enqueue(SAMPLE_CONTENT_VINKEN);
        testRunner.run();
        testRunner.assertTransferCount(RELATIONSHIP_UNMATCHED, 0);
        testRunner.assertTransferCount(RELATIONSHIP_SUCCESS, 1);
