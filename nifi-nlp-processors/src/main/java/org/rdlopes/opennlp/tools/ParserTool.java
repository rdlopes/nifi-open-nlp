...
public class ParserTool extends NLPTool<ParserModel> {
    ...
    protected void evaluate(ProcessContext processContext, InputStream content, Charset charset, Map<String, String> attributes, ParserModel model, Map<String, String> evaluation) {
        int numParses = NLPProperty.PARSER_PARSES_COUNT.getIntFrom(processContext);
        int beamSize = NLPProperty.PARSER_BEAM_SIZE.getIntFrom(processContext);
        double advancePercentage = NLPProperty.PARSER_ADVANCE_PERCENTAGE.getDoubleFrom(processContext);
        ...
        NLPAttribute.set(NLPAttribute.PARSER_PARSES_LIST_KEY, evaluation, parseCollection);
    }
    ...
    protected ParserModel trainModel(ValidationContext validationContext, InputStreamFactory inputStreamFactory, TrainingParameters trainingParameters, String trainingLanguage) throws IOException {
        final ParserType parserType = NLPProperty.PARSER_PARSER_TYPE.getEnumFrom(validationContext, ParserType.class);
        final String headRulesFilePath = NLPProperty.PARSER_HEAD_RULES_FILE_PATH.getStringFrom(validationContext);
